package com.softhex.timelineview;
...
public class TimelineView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        mMarkerText = typedArray.getString(R.styleable.TimelineView_markerText);
        mMarkerTextSize = typedArray.getDimensionPixelSize(R.styleable.TimelineView_markerTextSize, Utils.dpToPx(20F, getContext()));
        mMarkerTextColor = typedArray.getColor(R.styleable.TimelineView_markerTextColor, getThemeColor(R.attr.colorAccent, getContext()));
        mMarkerSize = typedArray.getDimensionPixelSize(R.styleable.TimelineView_markerSize, Utils.dpToPx(20F, getContext()));
        ...
        mMarkerColor = typedArray.getColor(R.styleable.TimelineView_markerColor, getResources().getColor(android.R.color.white));
        mLineWidth = typedArray.getDimensionPixelSize(R.styleable.TimelineView_lineWidth, Utils.dpToPx(2F, getContext()));
...
/*        if (isInEditMode()) {
        ...
        if(mMarkerText == null || mMarkerText == ""){
            mMarkerText = " ";
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mMarker != null) {
            mMarker.draw(canvas);
        }/*else if (mMarkerText != null && mMarkerText != "") {
            Paint textPaint = new Paint();
            textPaint.setColor(mMarkerTextColor);
            textPaint.setTextAlign(Paint.Align.CENTER);
            textPaint.setTextSize(mMarkerTextSize);
            int xPos = (canvas.getWidth() / 2);
            int yPos = (int) ((canvas.getHeight() / 2) - ((mMarkerTextSize / 2) - 110) * -1);
            canvas.drawText(mMarkerText, xPos, yPos, textPaint);

        }else{
            Bitmap mBitmap = Bitmap.createBitmap( canvas.getWidth(), canvas.getHeight(), Bitmap.Config.ARGB_8888);
            mMarker.draw(canvas);

            try{
                FileOutputStream fos = new FileOutputStream(mBitmapFile);
     ...
     * Sets marker.
     *
     * @param mText will set marker text to timeline
     */
    public void setMarkerText(String mText) {
        mMarkerText = mText;
        initTimeline();
    }
     ...
     * Sets marker.
     *
     * @param mBitmap will set marker text to timeline
     */
    public void setMarker(Bitmap mBitmap) {
        mMarkerBitmap = mBitmap;
        initTimeline();
    }

    public void setMarker(String mFile) {
        mBitmapFile = mFile;
        initTimeline();
    }
     ...
     */
    public void setMarkerTextColor(int mColor) {
        mMarkerTextColor = mColor;
        initTimeline();
    }
